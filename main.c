...
int main()
{
   /*
    GameStack stack;
    init_game_stack(&stack, board);

    make_move_with_history(board, 0x14, 0x34, &stack); // e2 → e4
    display_board(board);
 ...
 */
    ...
    while (1)
    {
        ...
        printf("\n%s to move. Enter move (e.g., e2 e4) or'undo' or 'redo' or 'quit': ",
        ...
        if (strcmp(from_str, "undo") == 0 || strcmp(from_str, "UNDO") == 0)
        {
            printf("Undo move.\n");
            undo(board, &stack);
            printf("\nAfter undo:\n");
        ...
        if (strcmp(from_str, "redo") == 0 || strcmp(from_str, "REDO") == 0)
        {
            printf("Redo move.\n");
            redo(board, &stack);
            printf("\nAfter redo:\n");
        ...
        if (make_move_with_history(board, from, to,&stack))
        {
            printf("Move %s to %s successful!\n", from_str, to_str);
            move_count++;
            display_board(board);

            // After make_move_with_history, the turn switches internally.
